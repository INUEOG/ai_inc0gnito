...
SECRET_PATTERNS: list[tuple[str, str, re.Pattern[str]]] = [
    ...
    ("dotenv_access", ".env token", re.compile(r"(^|[^A-Za-z0-9_])\.env([^A-Za-z0-9_]|$)|dotenv", re.I)),
    ...
    ("cloud_credentials", "cloud credential", re.compile(r"(GOOGLE_APPLICATION_CREDENTIALS|AZURE_CLIENT_SECRET|client_secret|access_key)", re.I)),
...
NETWORK_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ("external_post", re.compile(r"\b(curl|fetch|axios\.post|requests\.post)\b.*\b(POST|--data|-d|body\s*:|http://|https://)", re.I)),
...
def _scan_virtual_line(path: str, line_no: int, line: str) -> list[Evidence]:
    ...
    for evidence_type, pattern in DANGEROUS_EXEC_PATTERNS:
        if pattern.search(line):
            ...
            category = "remote_execution" if evidence_type == "remote_script_execution" else "obfuscation"
